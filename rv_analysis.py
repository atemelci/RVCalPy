...
def cmd_normalize(args):
    ...
    if bary is not None:
        if bary["instrument"]:
            hdr_lines.append(f"INSTRUME = {bary['instrument']}")
...
def cmd_batch(args):
    ...
    for p in files:
        try:
            checks.append(barycentric_frame_check(p))
        except Exception:
            pass
    if checks:
        n_cor = sum(c["verdict"] == "corrected" for c in checks)
        n_not = sum(c["verdict"] == "not-corrected" for c in checks)
    ...
    with open(outfile, "w") as f:
        ...
        f.write("# barycentric correction applied: "
                + ("yes" if apply_bary else "no") + "\n")
        if checks:
            nc = sum(c["verdict"] == "corrected" for c in checks)
            nn = sum(c["verdict"] == "not-corrected" for c in checks)
