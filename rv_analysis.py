...
WILSON_NSIGMA = 3.0             # outlier threshold, in robust sigma
...
def wilson_solve(rv1, err1, rv2, err2, labels=None, nsigma=WILSON_NSIGMA,
    ...
    outliers = (np.abs(resid) > nsigma * scale) & ~ambiguous
    ...
    return dict(
        ...
        outliers=outliers, swapped=swapped, ambiguous=ambiguous,
...
def make_wilson_figure(sol, title=None, cols=None):
    ...
    ok = ~(sol["outliers"] | sol["swapped"] | sol["ambiguous"])
    ...
    bad = sol["outliers"] & ~sol["swapped"]
    if bad.any():
        ax.plot(rv2[bad], rv1[bad], "x", ms=9, c="C1", mew=2,
                label="off the line")
    ...
    if sol["swapped"].any():
        axr.plot(np.arange(sol["n"])[sol["swapped"]],
                 ...
                 zorder=6)
    if bad.any():
        axr.plot(np.arange(sol["n"])[bad], sol["resid"][bad], "x", ms=9,
                 c="C1", mew=2, zorder=7)
...
def cmd_wilson(args):
    ...
    if bad.any():
        L.append("")
        L.append(f"Off the line by more than {args.nsigma:g} sigma:")
        for lab, r, o in zip(sol["labels"], sol["resid"], bad):
            if o:
                L.append(f"  OUTLIER  {lab}  {r:+.4f} km/s")
    ...
    for k in range(sol["n"]):
        flag = ("SWAPPED" if sol["swapped"][k]
                else "NEAR-CONJ" if sol["ambiguous"][k]
                else "OUTLIER" if sol["outliers"][k] else "ok")
...
def run_cli():
    ...
    p_wil.add_argument("--nsigma", type=float, default=WILSON_NSIGMA,
                       help="Distance from the line, in robust sigma, at "
                            "which an epoch is called an outlier "
    ...
    p_orb.add_argument("--nsigma", type=float, default=WILSON_NSIGMA,
                       help="Outlier threshold of the Wilson step")
